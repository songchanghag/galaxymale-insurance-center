...
<main class="admin-shell">
  <section class="admin-login" id="admin-login">
    ...
    <button class="btn-primary" type="button" onclick="loginAdmin()">로그인</button>
...
function setAdminView(isActive){ const login = document.getElementById('admin-login'); const panel = document.getElementById('admin-panel'); login.hidden = isActive; panel.hidden = !isActive; login.style.display = isActive ? 'none' : ''; panel.style.display = isActive ? 'grid' : 'none'; if(isActive) renderAdminLists(); }
function loginAdmin(){ if(document.getElementById('admin-password').value === 'galaxymale2026'){ localStorage.setItem('gmAdminSession','active'); localStorage.setItem('gm_admin_session','true'); setAdminView(true); } else alert('비밀번호가 올바르지 않습니다.'); }
function logoutAdmin(){ localStorage.removeItem('gmAdminSession'); localStorage.removeItem('gm_admin_session'); location.reload(); }
...
setAdminView(localStorage.getItem('gmAdminSession') === 'active' || localStorage.getItem('gm_admin_session') === 'true');
</script>
</body>
</html>`);
